...
const CinematicSculpture3D = ({ scrollProgress }: { scrollProgress: any }) => {
  ...
  useFrame((state) => {
    ...
    zoomProgress.current = Math.min(1, zoomProgress.current + state.delta * 0.5);
  }
    ...
    const alignX = isMobile ? 0 : rightX * 2.8;
    const scaleFactor = isMobile ? 0.6 : 1.0; // Shrink geometries by 40% on mobile
    ...
    if (geodeRef.current) {
      ...
      geodeRef.current.children.forEach((child: any) => {
        if (child.userData && child.userData.dir) {
          ...
          child.position.x = dir.x * (1.0 + pFracture * 1.6);
          child.position.y = dir.y * (1.0 + pFracture * 1.6);
          child.position.z = dir.z * (1.0 + pFracture * 1.6);
          // Individually spin each chunk for realistic floating look
          child.rotation.x = t * 0.35 + dir.x * 2.0;
          child.rotation.y = t * 0.25 + dir.y * 2.0;
        }
      });
  ...
  return (
    <group ref={groupRef}>
      ...
      <group ref={puddleRef} scale={0}>
        ...
        <mesh position={[1.5, 1.2, 0]}><sphereGeometry args={[0.08, 32, 32]} /><meshStandardMaterial color="#c2a688" roughness={0} metalness={1} clearcoat={1} /></mesh>
        <mesh position={[-1.2, 0.8, 1.0]}><sphereGeometry args={[0.05, 32, 32]} /><meshStandardMaterial color="#c2a688" roughness={0} metalness={1} clearcoat={1} /></mesh>
        <mesh position={[0.5, 1.5, -1.5]}><sphereGeometry args={[0.1, 32, 32]} /><meshStandardMaterial color="#c2a688" roughness={0} metalness={1} clearcoat={1} /></mesh>
        <mesh position={[-0.8, 2.0, -0.5]}><sphereGeometry args={[0.06, 32, 32]} /><meshStandardMaterial color="#c2a688" roughness={0} metalness={1} clearcoat={1} /></mesh>
      </group>
    </group>
  );
};
...
const PortfolioCard3D = ({ index }: { index: number }) => {
  return (
...
export default function App() {
  ...
  return (
    <div className="bg-luxury-black text-[#f5f2eb] relative font-sans selection:bg-luxury-gold/30 overflow-hidden">
      ...
      <header className="fixed top-0 w-full z-50 py-4 md:py-6 bg-transparent" style={{ mixBlendMode: 'screen' }}>
        <div className="max-w-7xl mx-auto px-6 md:px-16 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-12 text-[10px] uppercase tracking-[0.3em] text-white/60 font-medium">
            {['ABOUT', 'WORK', 'CREATIONS', 'PROCESS', 'CONTACT'].map((item) => (
              <a 
                key={item} 
      ...
      <section id="hero" className="relative h-screen flex items-center overflow-hidden">
        <div className="relative z-20 max-w-7xl mx-auto px-6 md:px-16 w-full flex justify-between items-center h-full pt-32 md:pt-44">
          <div className="max-w-md mt-24 md:mt-32">
            <motion.div
              initial={{ opacity: 0, y: 40 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1.4, ease: [0.16, 1, 0.3, 1] }}
            >
              <span className="text-[10px] tracking-[0.3em] text-luxury-gold uppercase font-mono mb-6 block">PHASE 01 — DIGITAL ORIGIN</span>
              <h1 className="text-5xl md:text-[5.5rem] font-serif font-light leading-[1.05] tracking-tight mb-8">
                Premium digital <br />
                experiences that <br />
                drive <span className="text-luxury-gold italic font-normal">growth.</span>
              </h1>
              ...
              <MagneticButton className="group flex items-center gap-4 px-8 py-3.5 border border-white/10 rounded-full hover:bg-luxury-gold hover:text-black hover:border-luxury-gold transition-all duration-500 bg-white/[0.01]">
                <span className="text-[10px] uppercase tracking-widest font-semibold">Start Your Project</span>
      ...
      <section id="about" className="relative py-48 px-6 md:px-16 border-t border-white/5">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-16 items-center">
          <div className="lg:col-span-5">
            <span className="text-[10px] tracking-[0.3em] text-luxury-gold uppercase font-mono mb-6 block">PHASE 02 — BREAKING BOUNDARIES</span>
            <h2 className="text-4xl md:text-6xl font-serif font-light leading-tight mb-8">
              Shattering <br />
              traditional <span className="text-luxury-gold italic font-normal">web design.</span>
            </h2>
            ...
            <MagneticButton className="group flex items-center gap-4 px-8 py-3.5 border border-white/10 rounded-full hover:bg-luxury-gold hover:text-black hover:border-luxury-gold transition-all duration-500 bg-white/[0.01]">
              <span className="text-[10px] uppercase tracking-widest font-semibold">View Case Studies</span>
              <ArrowRight size={12} className="group-hover:translate-x-2 transition-transform duration-300" />
            </MagneticButton>
          </div>
      ...
      <section id="services" className="relative py-48 px-6 md:px-16 bg-[#201d18]/10 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-12 gap-16 items-center mb-16">
            <div className="lg:col-span-5">
              <span className="text-[10px] tracking-[0.3em] text-luxury-gold uppercase font-mono mb-6 block">PHASE 03 — STRATEGIC ECOSYSTEMS</span>
              <h2 className="text-4xl md:text-6xl font-serif font-light mb-8">
                Engineered for <span className="text-luxury-gold italic font-normal">business scaling.</span>
              </h2>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {[
              { title: 'INTERACTIVE UI', desc: 'Next-generation frontend development.' },
              { title: 'PERFORMANCE', desc: 'Lightning-fast load times for better retention.' },
              { title: 'SEO MASTERY', desc: 'Built to rank and drive organic traffic.' },
              { title: 'SCALABILITY', desc: 'Robust architecture ready for your business growth.' }
            ].map((service, idx) => (
              <div key={service.title} className="p-10 border border-white/5 rounded-xl bg-white/[0.01] hover:border-luxury-gold/25 hover:bg-white/[0.02] transition-all duration-700 group">
                <span className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center mb-8 text-[10px] text-luxury-gold font-mono">
                  {idx + 1}
                </span>
                ...
                <p className="text-white/40 text-xs font-light leading-relaxed">{service.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="work" className="relative py-48 px-6 md:px-16 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-24">
            <div>
              <span className="text-[10px] tracking-[0.3em] text-luxury-gold uppercase font-mono mb-6 block">PHASE 04 — PROVEN RESULTS</span>
              <h2 className="text-4xl md:text-6xl font-serif font-light leading-none">
                Solidifying your <br />
                digital <span className="text-luxury-gold italic font-normal">presence.</span>
              </h2>
            </div>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {[
              { title: 'ECOMMERCE', type: 'High Conversion' },
              { title: 'SAAS PLATFORM', type: 'Interactive Dashboard' },
              { title: 'CORPORATE SITE', type: 'Brand Authority' },
              { title: 'PORTFOLIO', type: 'Creative Showcase' }
            ].map((p, idx) => (
              <div key={p.title} className="group cursor-pointer">
                <div className="aspect-[3/4] relative mb-6">
                  <PortfolioCard3D index={idx} />
                </div>
                <div className="flex justify-between items-center px-1">
                  <h3 className="text-lg font-semibold tracking-widest group-hover:text-luxury-gold transition-colors">{p.title}</h3>
                  <span className="text-xs tracking-widest text-white/40 uppercase font-mono">{p.type}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="creations" className="relative py-32 px-6 md:px-16 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <div className="group cursor-pointer">
              <div className="relative overflow-hidden rounded-xl border border-white/10 aspect-video mb-6">
                <video src={manacVideo} autoPlay loop muted playsInline className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
            ...
            <div className="group cursor-pointer">
              <div className="relative overflow-hidden rounded-xl border border-white/10 aspect-video mb-6">
                <video src={restaurantVideo} autoPlay loop muted playsInline className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors duration-500"></div>
              </div>
              <div className="px-2">
      ...
      <section id="process" className="relative py-24 px-6 md:px-16 bg-[#201d18]/10 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-12 gap-16 items-center mb-16">
            <div className="lg:col-span-5">
              <span className="text-[10px] tracking-[0.3em] text-luxury-gold uppercase font-mono mb-6 block">PHASE 05 — FLUID COLLABORATION</span>
              <h2 className="text-4xl md:text-6xl font-serif font-light mb-8">
                A seamless, tailored <span className="text-luxury-gold italic font-normal">process.</span>
              </h2>
              <p className="text-white/40 text-base font-light leading-relaxed max-w-md">
      ...
      <section id="contact" className="relative pt-16 pb-32 px-6 md:px-16 border-t border-white/5 text-center bg-transparent">
        ...
        <div className="max-w-4xl mx-auto relative z-10 mt-32">
          <span className="text-[10px] tracking-[0.3em] text-luxury-gold uppercase font-mono mb-6 block">PHASE 06 — ACCELERATION</span>
          <h2 className="text-4xl md:text-7xl font-serif font-light leading-none mb-12">
            Ready to elevate <br />
            your <span className="text-luxury-gold italic font-normal">business?</span>
          </h2>
          <MagneticButton 
